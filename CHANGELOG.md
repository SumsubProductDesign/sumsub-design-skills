...
- **The component is SLOT-based now.** Rows live in a `"Rows wrapper"` SLOT, cells in a per-row `"Row"` SLOT, header cells in a `"Cell wrapper"` SLOT. `table.children.filter(c => c.name === "Table Row")` returns **0**, and `tableHeader.children` returns the SLOT, not the cells. The structure diagram, both code samples and the `Table Row Structure` block in SKILL.md were rewritten against the verified tree (component-set keys for `Table Row`, `Table Row / Cell Content`, `.Table Header / Heder Cell Content` are now recorded).
- **New rule "0. Node-handle lifecycle".** A handle dies immediately after its own write and takes the cached `page`/`section`/`table` ancestors with it. Re-derive the whole chain from `figma.root` before *every* mutation. Four consequences documented and verified: (1) **one write per row-SLOT per `use_figma` call** — fill one column per call; (2) a VARIANT write rebuilds the node, so other keys in the same `setProperties` hit a dead node; (3) `.visible = false` on a slot child **removes it from `children`**, shifting indices — target by measured width, not a captured index; (4) `figma.notify()` output never reaches the agent, so counters are invisible — read back with a separate read-only call.
- **"Column count — via DS property" was wrong and is replaced.** Probed: `table.componentProperties` exposes only `Rows wrapper` (SLOT), `Toolbar` (BOOLEAN), `State` (VARIANT). There is **no** column-count property to discover, so the old instruction dead-ended at "leave the column count as-is" and shipped a stray checkbox column plus an empty trailing cell. Hiding slot cells does work — the previous blanket ban was wrong; what breaks builds is hiding them **asymmetrically** (observed header 5×227px against rows 5×200px, so every label sat over the wrong column).
- **Audit check 7.60 — table-column-misalignment.** Compares visible header cell `x`/`width` against every visible row's cells and fails on a mismatch. This is the exact defect the build shipped and had to repair; checks 1–7.59 all passed on it (no TEXT leak, no overflow, no hidden content, no empty body).
- **Audit parts rebalanced.** `audit-part3.js` had reached **49 943 bytes** — 57 bytes under the `use_figma` `code` cap of 50 000, i.e. the next check of any size would have made the verbatim run impossible. The 7.44 Case-page block moved to part 2; parts are now 36 / 46.5 / 42KB. SKILL.md's run protocol notes that the ranges are descriptive and that the 50KB ceiling is the real constraint.

Also recorded: `get_libraries` currently fails with an SSE JSON-parse error (response truncated ~30KB) — verify library availability empirically by importing component keys instead, and say so in the build log rather than claiming the libraries are unavailable.

## v3.202.0 — 2026-07-29 (GitLab MR review fixes: 7 skills not 8; audit.js duplicate removed; docs-dump provenance headers)
Fixes from the internal-marketplace MR !43 review. (1) **Skill count corrected to 7** — `sumsub-local-component` was removed in an earlier version but INSTALL.md (and the GitLab plugin README) still promised it; bullet + command-table row dropped. (2) **`reference/products/audit.js` (120KB) deleted** — it duplicated the three run-path segments `audit-part1/2/3.js` and could only drift; the part files are now the source of truth (SKILL.md §self-verify updated: run the three parts in order; edit parts directly, keep each <50KB). (3) **Provenance headers added to all 7 `sumsub-docs-*.txt` dumps** — they are text dumps of PUBLIC docs.sumsub.com documentation (the JWT-looking strings inside are documentation examples, not secrets); the dumps stay because they're load-bearing (SKILL.md's product-keyword table mandates reading them for product terminology and realistic content).

## v3.201.0 — 2026-07-29 (all documentation translated to English)
All plugin documentation and skill instructions translated from Russian to English for the corporate GitLab marketplace migration (user request after reviewing the MR diff). 21 files: every SKILL.md, all reference/product docs (incl. design-system.md — 217 lines, BLOCKS.md rewritten wholesale), ONBOARDING.md, and the Russian user-feedback quotes inside CHANGELOG entries (attributed as `user feedback: "..."`). Preserved byte-identical: Figma component/variable keys, node-ids, fileKeys, variant/property/token names, hex values, code, table shapes. Deliberately KEPT in Russian (with English glosses): 18 strings that are matching patterns — banned-phrase exemplars the skills detect in agent output and user-prompt trigger phrases (e.g. "все экраны" ("all screens")) — translating those would break the matching. No behavior changes.

## v3.200.0 — 2026-07-28 (31 reference paths in SKILL.md pointed at files that did not exist; audit could not resolve its own root)
Found while building the APRD-756 Applicant-page mockup. Two structural defects, both of which had been silently degrading every run:

- **The whole `reference/products/` path table was broken.** SKILL.md refers to `${CLAUDE_PLUGIN_ROOT}/reference/products/…` in 69 places, but the maintained docs physically lived in `skills/sumsub-mockup/reference/products/`. A survey of every `${CLAUDE_PLUGIN_ROOT}` path across all skills returned **31 MISSING of 53** — every product pattern doc plus all three `audit-part*.js` and `audit.js`. This is the real cause of the recurring "pattern doc doesn't exist / 404" reports that the v3.166 rule was written to suppress: the agents were right, the paths were wrong. Docs moved to `reference/products/`; survey now returns **0 MISSING**.
- **8 stale root shadows deleted.** `reference/applicant-page-pattern.md` (12.4KB, frozen 2026-04) shadowed the maintained 20.2KB copy — and it was the file the Applicant-page obligatory-read row pointed at, so an agent following SKILL.md read a doc that still teaches the **52px AP Sidebar that audit 7.46 bans**. Same drift in `design-system.md` (102KB vs 113KB), `BLOCKS.md`, `color-usage.md`, `layout-patterns.md`, `tm-layout-patterns.md` (+2 identical). The 15 SKILL.md rows that pointed at root copies now point at `reference/products/`. No duplicate `.md` basenames remain in the reference tree.
- **Audit root resolution was sync.** `audit-part1|2|3.js` + `audit.js` opened with `const root = figma.getNodeById("ROOT_ID_HERE")`. Sync `getNodeById` only resolves nodes on `figma.currentPage`, which is whatever the desktop app has active and **resets between `use_figma` calls** — so a build on the Drafts page crashed the audit with `cannot read property 'parent' of null` on the very next line, pushing agents to hand-fix the "verbatim" script or skip the audit. Now `await figma.getNodeByIdAsync(...)` (the script body runs in an async context, so top-level await is fine). Verified empirically from a foreign current page: old sync → `null`, new async → resolved, `page` derived, 171 nodes walked.

Also indexed: the **full-body Applicant page canonical** is `Di7nvHaOxXiWuDAN1oa0hK` page "Layout", frame `4199:389967` (Page header 1440×152 + Summary 360 + Body 1080×**10982**, 24 sections). The frames the pattern doc cites (`17501:30301`, `13395:21886`) are short feature demos with 7–9 sections. Current prod `Profile information` block = `d00e78d99d9f1d1f21f090e9b74b669c0cfc1286`; the legacy `Profile data` card in the canonical is a different, superseded block.
...
Account re-run at v3.196: authority order landed (canonical `11061:178283` Brand updates, geometry 7.45 match, 0 leaks) — user feedback: "already better" — but the build shipped **desktop only**. The standing "every screen = Desktop+Mobile pair" rule was encoded in websdk-mockup SKILL.md; sumsub-id had only a passing note → the rule didn't fire. Fix: explicit D+M block in sumsub-id SKILL.md Step 3 (mobile canonicals = the `steps` 375-frames in the same Brand-updates section; never a resized desktop). Per-skill rule drift class again.

## v3.196.0 — 2026-07-20 (sumsub-id: Account canonical authority order — Brand updates page; old canonical is ARCHIVED)
First sumsub-id-mockup sim (Account dashboard): geometry/components/data mostly fine, but the screen look was outdated — user feedback: "The skill didn't fully pull it off — it clearly doesn't know what the different account-dashboard screens should look like". The Account file went through a **brand update**: the 2026-04 canonical page "Sumsub ID account" (`3416:14299`, still listed as PROD in the pattern doc) is now in **💀 Archive**, and the skill built from `⭐️ Master Flow`'s pre-brand `Main-Have docs` while the brand-updated version (`11061:178283`) existed. Fix: `sumsub-id-pattern.md` — fresh page inventory (2026-07-20) + **canonical AUTHORITY ORDER** (`[*️⃣ dev] Brand updates` → `[❇️ PROD]` feature pages → Master Flow last-resort-flagged → Archive NEVER) + **Brand updates screen catalog** with node-ids (Welcome / Code ×4 / Creating account ×4 / Account main-noDocs `11061:178274` + Main-Have docs `11061:178283`, each with mobile `steps` 375-frames as the M canonicals). SKILL.md Step 3 carries the order.

## v3.195.0 — 2026-07-20 (#8c: purge the Slot's DEFAULT content — the master ships Steps NEW with the Sumsub-ID promo)
Flow run №3 (user feedback: "why did it stuff the sumsubid block into every mockup?"): 6 of 9 screens rendered the "Get verified faster with Sumsub ID" promo under their real content. Cause: the `Widget[Type=Content]` master's `Slot` ships **`Steps NEW`** (steps overview containing `SNS ID | Desktop` Active=True) as DEFAULT content; the skill inserted each step's organism but never purged the default → it rendered below. Canonical shows that block on exactly ONE screen — Steps overview (`9196:199933`). Run №2 had flagged and purged the same strays ad-hoc; without a recipe step it regressed. Fix: **#8c** in `examples-library.md` — when filling ANY slot, remove ALL pre-existing slot children (refs before insertChild, remove by ref after); audit: rendered `Steps NEW`/`SNS ID`/promo text outside the Steps-overview screen = FAIL. kyc-flow-map: new "Sumsub ID promo — ONLY on Steps overview" section. Build repaired in-file (6 strays removed, 0 promo texts render).

## v3.194.0 — 2026-07-20 (#8b: assert the slotted instance's VARIANT — Image Slot must match the step)
Flow re-run: sequence/tips-pairing/texts all correct (v3.193 landed), but user feedback: "Again it substituted the wrong tips screen before the document upload" — the doc-tips screen carried **`Image Slot[Type=Liveness]`** (liveness illustration) while the canonical `3044:140189` has `Image Slot[Type=ID]`; every text matched, so text/visibility audits passed and the screen still read as the wrong one. Root cause: slotted-instance VARIANTS don't follow the content organism and were never set/checked. Fix: **#8b** in `examples-library.md` + kyc-flow-map Tips section — for every FILLED slot, set AND audit the slotted instance's variant (`mainComponent.name`) against the canonical (Image slot mapping: doc-ID→`Type=ID`, passport→`Type=Passport`, liveness→`Type=Liveness`); post-`setProperties` refs are invalidated — re-fetch before verifying. Build repaired in-file (doc-tips D+M → `Type=ID`).

## v3.193.0 — 2026-07-20 (FLOW understanding: sequence from the flow canonical, per-capture Tips pairing; new kyc-flow-map.md)
First FLOW sim (full KYC flow + annotations): screens/annotations/pairs assembled fine, but flow SEMANTICS were wrong (user feedback: "it used the wrong tips screen before the document capture, and didn't place one before liveness at all") — the skill built literally from the prompt's step list: doc-tips assembled by guess (illustration "best-effort swapped") and NO tips before Liveness. Ground truth found: **flow canonical `Lvs3OA19oILUfjyairhb81` "[✳️ PROD] General"** → section "General flow" (WEB `7130:32504` + mWEB `9227:299668`) — 17-screen KYC sequence with `Scenarios` annotations built in. New `reference/kyc-flow-map.md`: THE FLOW RULE (prompt names anchor steps; canonical supplies the in-between screens), the full sequence table, per-capture Tips pairing (doc ← `Tips[ID | Live Capture]`+`Image Slot[ID]`, per-doc-type variants ID `3044:140189`/Passport `3044:155375`; liveness ← **`Tips[Type=Selfie]`**+`Image Slot[Liveness]` "Let's take a selfie" — NOT `Tips | Liveness`; Guidelines = separate organism), flow presentation (Scenarios row, D+M rows, uniform 1440×980 wrappers are canonical for flows). SKILL.md: flow rule + reference listing. Also fixed a stale self-contradiction: "KYB does NOT use the Widget shell" claim (disproved in v3.171) still lived at the reference listing — rewritten.

## v3.192.0 — 2026-07-16 (websdk: ensureDraftsPage must CREATE the Drafts page — no children[0] fallback; audit checks the page)
Kostya asked how the skill was "getting around" the Drafts-only rule. It wasn't bypassing — it was executing a degraded copy: websdk-mockup's inline `ensureDraftsPage` snippet fell back to `figma.root.children[0]` when no `/drafts/i` page exists (the test file has a single "Onboarding" page → every WebSDK build landed there), while sumsub-mockup's `helpers.js` correctly CREATES `🛠 Drafts`. Nothing flagged it: page-check 7.12 lives in the sumsub-mockup audit; the websdk `auditWidget` never looked at the page. Per-skill drift class. Fix: websdk snippet now creates the page when missing (aligned with helpers.js); `auditWidget` gained a Drafts-page check (output on a non-Drafts page = issue, exceptions: user-named target page/section, in-place migration on the source page). Existing test-file sections stay on "Onboarding" (not moved).

## v3.191.0 — 2026-07-16 (Override #7a: organism sizing axes + Widget root dims from THIS canonical; every screen = Desktop+Mobile pair)
Liveness sim (user feedback: "The skill tried to assemble its own layout and got it wrong"): composition was 1:1 with canonical `2344:85996` (variant map, slot visibility, organism variant all matched) but the NUMBERS were self-assembled — Widget root sized 1440×**960** (carried from the previous Camera round; THIS canonical is 1440×**900**) and the organism's `layoutSizingVertical` left FIXED at 542 while the canonical has **FILL** (794) → 312px dead space under the camera. Fix: **Override #7a** in `examples-library.md` + SKILL.md override-map rule — mirror the organism's BOTH sizing axes (szH+szV) and the Widget root w/h from THIS canonical, never reuse dims from a previously built screen (they differ even within the same Widget variant); audit diffs root dims + sizing axes. Build repaired in-file (900 / szV=FILL). **NEW standing requirement:** every WebSDK screen request builds a **Desktop + Mobile pair** (each from its OWN canonical example, same wrapper section) unless the user explicitly asks for one platform.

## v3.190.0 — 2026-07-16 (page tabs = Header SUBHEADER, not standalone Tab Basic; tab items live in the Items-wrapper SLOT — findAll)
First external-designer sim (Applicants approved list, Admin area file) surfaced two defects the skill itself prescribed. (1) **Tabs placed as a standalone `*Tab Basic*` row in Content** — `blocks/table-page.js` step 7 literally appended it under the Title Row; user feedback: "The tabs are not a subheader (the way they should be)". Correct: enable `Subheader#4002:0=true` on the `*Header*` instance — its `Header / Subheader` carries the `*Tab Basic*`. Standalone Tab Basic is only for tabs inside a card/panel body. (2) **Default `Tab_1…Tab_5` labels initially rendered** — SKILL.md's own example fetched items via `tabs.children.filter(...)`, but the items sit inside the `Items wrapper` SLOT, so the filter returns nothing and the config loop no-ops. Fixed to `findAll` everywhere (SKILL.md §Tab Basic — new failure mode #3, table-page.js block rewritten, BLOCKS.md structure updated). Audit 7.42 extended: a `*Tab Basic*` that is a direct child of the page Content frame = FAIL.

## v3.189.0 — 2026-07-16 (Widget Override #8 — mirror EVERY slot's visibility from the canonical, variant-agnostic)
Camera sim (first `Type=Camera` build): content correct (variant, 1440×960, organism `POI camera Desktop` in Camera slot, 0 leaks, placement ✓) BUT the build rendered a left navigation + logo the canonical doesn't have — the `Type=Camera` master ships `Left side` and `slot` (logo) VISIBLE by default; the canonical example (`2344:108816` → `2344:108817`) hides both. The 7-override recipe names only the Content-variant frames (`instruction`/`Image`/`Left bar`), so the skill neither hid nor CHECKED the Camera-specific slots — the audit signature simply omitted them → false PASS. Fix: **Override #8** in `examples-library.md` + SKILL.md override-map rule: enumerate ALL SLOT nodes of the assembled Widget and set each `visible` to the canonical's effective (ancestor) visibility, 1:1 whatever the slot is called; the audit must assert per-slot rendered-state equality for EVERY slot (a slot missing from the signature = unchecked, not passing). Build repaired in-file (both slots hidden on 1319:127269).

## v3.188.0 — 2026-06-15 (placement-into-section rule propagated to ALL skills; relative-vs-absolute coord confusion)
WebSDK Welcome-Mobile sim: content PASS (Widget shell, Small top bar, organism FILL, black button, 0 leaks) but the frame landed **on top of an unrelated old mockup** — the skill computed section-relative coords (x=1560, even placed a `Mobile · 375×812` label inside the section there) yet appended the frame to the **PAGE**, where 1560 is an absolute canvas coordinate. Same class as v3.186 (Section does not adopt by overlap), now with the coord-space confusion made explicit. v3.186 encoded the rule only in `island-layout-pattern.md` (per-skill rule drift). Fix: placement-into-section block added to **websdk-mockup** SKILL.md (appendChild FIRST → coords become relative → free-spot = right of rightmost child + gap → grow section if needed → verify parent===section, no overlap, bounds enclose) and to **sumsub-mockup** SKILL.md §7.7 (sections rule); sumsub-id-mockup inherits sumsub-mockup. File repaired by hand (mobile frame moved into section 1282:140389 at (1560,160), overlap cleared).

## v3.187.0 — 2026-07-08 (component tokens on custom nodes stay; batch token-fix gotchas)
Batch semantic-token fix on New Layout CM page surfaced a rule gap: design-review treated ANY non-semantic binding on a custom node as a violation, so 6 deliberate `status/green/*` (component-token) bindings on custom badge pills got rebound to their semantic alias targets — user feedback: "if specific component variables are applied, they must not be replaced with semantic ones — only base or hex values should be changed" (reverted). Token Layer table now has an explicit LEAVE-AS-IS row for component tokens (`status/*`, `components/*`, `button/*`, `field/*`…) applied on custom nodes — only `base/*` and raw hex are violations. Also added batch-fix gotchas: walk per-section (full-page walk drops the MCP transport), data-viz fills stay on base palette (no same-hex semantic exists), bind spacing/radius only on exact-value tokens (no tokens for 1/2/10/20/40/64 spacing, 2/40/100 radius), watch capitalized duplicate collections (`Base/Neutral/90`).

## v3.186.0 — 2026-06-15 (copy/build output must live INSIDE its wrapper section — appendChild, not page-sibling)
The KYC "Configurations" build (v3.185, otherwise correct — Create level now renders on all 5) created a `(made by Claude)` wrapper Section but left the 5 `Page` instances as page-level siblings BESIDE it, so the section looked empty and the user asked "does the skill think it actually did something?". A Figma Section does not adopt frames by geometric overlap — you must explicitly `section.appendChild`. Fixes: §5 placement rule (create section → `appendChild` each screen → row layout with fixed gap → confirm section bounds enclose all screens) + §7 audit check (every produced screen is a `child` of the wrapper section, `section.children.length === screenCount`, bounds enclose all; empty section with screens beside it = FAIL). Island migration/build is now complete end-to-end (instance, header incl. carried actions, content in Main, overlays, height, sandbox, placement).

## v3.185.0 — 2026-06-15 (build-from-reference = copy-mode migration; don't hand-roll a build that drops the header action buttons)
The KYC "Configurations" build (5 screens) surfaced a build-path gap: asked to "build island versions of these screens", the skill hand-rolled an assembly (clone body into a `Page` instance, set title + breadcrumb + tabs) and STOPPED — silently **dropping the header action buttons**. Result: `Create level` missing, `Actions slot` left **hidden** (its Button was the Page's own default placeholder, not the carried action); only the Page's default help icon rendered. The action-carry (`Show actions slot=true` + carry the original's ancestor-visible actions) lives only inside `migrateFrameToIsland`. Fixes: §0 + §5 now state explicitly that "build/create island versions of these screens" IS copy-mode migration — run `migrateFrameToIsland` per source frame, never hand-roll the header config. New §7 audit check: the carried primary action must be **ANCESTOR-VISIBLE** (assert the Actions slot RENDERS, not merely that a button sits in it) — a present-but-hidden Actions slot with the real action missing = FAIL. (Ancestor-visibility class again.)

## v3.184.0 — 2026-06-15 (header: carry the ⋮ 'More'/kebab too; drop the slot's default secondary 'Button')
v3.183 carried only TEXT actions, so the original's icon-only ⋮ 'More'/kebab was skipped and the Actions slot's default secondary `Button` placeholder stayed in its place (user feedback: "Instead of the 'More' kebab button it just left the default secondary button"). Inspection: the original action group ("Buttons"/"Buttons Bar") = Create level (Primary text) + kebab (`Content=Icon Only, Type=Secondary`, key 479ef206) + AI (icon Primary) + help (icon Tertiary); the Actions slot shipped a default `Button` (Secondary, key d54b0031) my placeholder-removal missed. Fix in `migrateFrameToIsland`: harvest from the action group both the text actions AND the icon-Secondary kebab (skip AI/help — Page header provides them), then drop any leftover slot child labelled exactly `Button`. §3 + §7 updated.
...
v3.182 carried buttons but over-carried (user feedback: "the skill crammed in everything it found in the header component instead of the real buttons from the original"): the harvest filtered by each button's OWN `.visible`, so it also cloned the old header's hidden **Key/applicant-area** buttons (ID / External ID / Add tag) and a placeholder labelled `Button`. Inspection of the original header showed the REAL actions live in the `Buttons / Buttons Bar` group and only `Create level` is ANCESTOR-visible; the junk has `.visible=true` but a hidden parent. Fix (ancestor-visibility class — recurred all session, cf. sandbox detection): `migrateFrameToIsland` now walks each candidate button's parent chain and carries ONLY ancestor-visible text actions, also excluding the literal `Button` placeholder. §3 + §7 updated. Island migration complete + the button carry-over is now precise.

## v3.182.0 — 2026-06-15 (header action-buttons carried onto the Page header — Create level renders in the cluster)
The migration now reproduces the original header's button structure (user feedback: "it's important that the skill also reproduces the header's button structure, not just swaps the header for a new one"). Validated mechanism, encoded in `migrateFrameToIsland` + §3 + §7: enable the Page header's `Show actions slot#6943:20=true`, clone each of the original's TEXT-labeled action buttons (`Create level` / `Run level` / `Save` — not the icon kebab/AI/help, which the Page header already has) into the header's **"Actions slot"**, then **set each clone `visible=true`** — clones arrive `visible=false`, which is why an earlier attempt put the button in the slot but it didn't render. Verified: `Create level` now renders in the header cluster alongside the icon actions. This was the last open island-migration TODO; the instance migration is now complete (instance ✓, header tabs+buttons ✓, content-not-split ✓, overlays-as-siblings ✓, height-preserved ✓, sandbox-variant ✓).

## v3.181.0 — 2026-06-15 (instance approach WORKED — fix the two bugs it exposed: content split + illegal overlay-append)
With v3.180 the skill finally **used the `Page` instance** (×13) — but broke the layout inside. Two function bugs, found by comparing a good prior migration (20692) vs the instance build (20778): (1) `migrateFrameToIsland` **split** the editor content into the Page's Main/Side columns (form→Main, overview→Side) → the form left-flushed in a shrunk 896 column instead of the original centered 640+380 pair; (2) the overlay step did `page.appendChild(toast)` — **illegal on a component instance** (children are locked) → it dropped the 4 toasts mid-run. Fixes in `island-layout-pattern.md` §2/§5/§6/§7: put the WHOLE content block into the **"Main content" slot with "Side content" left HIDDEN** (Main = 1340, fits the pair as positioned — do NOT split); place overlays as **SIBLINGS of the Page instance** (positioned over it), never appended onto it. Also documented the Figma gotcha that **`insertChild` into a SLOT invalidates the inserted node's reference** (re-fetch from `slot.children` for any follow-up edit — caused two rolled-back validation runs). Remaining TODO: carry the old header's action buttons onto the Page header (tabs already carried).

## v3.180.0 — 2026-06-15 (ROOT CAUSE of the repeated hand-build: the doc contradicted itself — §3 "flip the old header" vs §5 "use the Page's own header")
User insight ("we're doing something wrong; it doesn't even try to use the component"). The 4th sim's `method_note` finally gave the agent's real reason: it tried to keep/flip the OLD header to Version=New, saw applicant junk (ClientNickname/ID/Suspicious/default Button), concluded the Page instance was "incompatible with keeping the Old header", and hand-built. Root cause: `island-layout-pattern.md` §3 still taught the **pre-publish hand-build trick** ("flip the existing header's Version Old→New, never rebuild") which directly CONTRADICTS the instance path (§0/§2/§5: "use the Page's OWN header"). The agent followed §3 and bailed. Verified the truth: the Page instance's OWN Version=New header is CLEAN — `Levels / New level` + the original tabs (Steps/Configurations/Checks); the applicant junk appears ONLY when flipping the OLD applicant-context header. Setting `Key#5362:0=false` removes the lone stray 'Key name' badge → fully clean. Fix: §3 rewritten to "use the Page instance's OWN header — read the old header's labels (title, tab labels, button labels), apply them to the Page header, `Key=false`, then DISCARD the old header"; the Version-flip is now marked valid ONLY in the hand-build fallback. §5 function adds `Key=false`. This resolves the self-contradiction that drove the agent to hand-build for 4 sims.

## v3.179.0 — 2026-06-15 (give the EXACT validated instance-migration function to copy-run; prose rules failed 3×)
The skill hand-built the island a THIRD time despite the v3.177 hard rule + v3.178 essence (confirmed: `Page` imports fine from the file, yet it built raw frames + kept the Version=Old header). Prose rules don't override the agent's familiar prior. So I proved the full instance migration end-to-end myself and encoded the **exact validated function** (`migrateFrameToIsland`) in `island-layout-pattern.md` §5, instructed "COPY AND RUN THIS verbatim — do NOT write your own". Validated result per frame: live `Page` INSTANCE (`type==="INSTANCE"`), **original height preserved** (800/1130 — answers the open height question), content moved into `Main content`/`Side content` slots (placeholders removed via the safe capture-refs-before-insert pattern that avoids the stale-node throw), the Page's built-in Version=New header configured from the original (Title + breadcrumb + Subheader tabs relabeled to the originals: Steps/Configurations/Checks), overlays re-floated ABSOLUTE, sandbox via the `Sandbox` variant. Still TODO: carrying the original action BUTTONS onto the Page header (tabs done).

## v3.178.0 — 2026-06-15 (lead with THE ESSENCE: instantiate the whole-layout `Page` + variant + content-in-slot — 3 steps)
Added a `## 0. THE ESSENCE` block at the very top of `island-layout-pattern.md` so the mental model is the first thing read (the v3.177 hard rule was correct but buried under procedure): an island screen = (1) instantiate the WHOLE-layout `Page` component (sidebar+island+card+header are already inside it), (2) apply the right VARIANT (`Type` Basic/Full-screen + `Sandbox` No/Yes — the variant drives all the chrome), (3) put content into its slot (remove the default placeholder). Explicit tripwire: "if you find yourself calling `figma.createFrame()` for an Island/Body/Card — STOP, you're doing it wrong; instantiate `Page`." User feedback framed the essence: "it must use an instance of the whole-layout component, apply the right variant, and place the needed content inside".

## v3.177.0 — 2026-06-15 (HARD RULE: import-first, instance-MANDATORY — hand-building the island is BANNED unless the Page import actually throws)
Re-sim of the island migration: the lossless header (Version flip — tabs/buttons preserved), toast preservation, and sandbox-by-visibility all worked — BUT the skill **hand-built the island from raw frames again, even though `Page` imports fine** (confirmed: `importComponentSetByKeyAsync` returns OK from the file), calling hand-build "the fallback". This is the recurring "soft directive doesn't override the agent's familiar prior" class (cf. AP-sidebar, plugin-version sagas). Fix is mechanical enforcement, not a softer note: `island-layout-pattern.md` §2 + §7 + SKILL.md now make it a HARD RULE — the MANDATORY first step is `importComponentSetByKeyAsync(Page)`; **if it resolves you MUST instantiate; hand-building is BANNED**. Hand-build is permitted ONLY when that import THROWS and the build log pastes the actual error. The §7 audit now **re-tests the import itself**: `pageImportable && shell-is-hand-built-frames ⇒ FAIL` ("hand-built while Page was importable"). Banned rationalizations enumerated ("I'll hand-build / familiar path / fallback / the result looks the same / §7 passed"). NOTE from the sim: the Version flip is not 100% lossless for button-heavy editor headers (the New header's action/info slots need explicit restore) — to refine.

## v3.176.0 — 2026-06-15 (`Page` component PUBLISHED → instance+slot approach validated & encoded; sandbox = Page variant)
The user published the `Page` component, unblocking v3.175.0. Re-verified importable; validated the instance recipe end-to-end. `island-layout-pattern.md` (§2 build approach, keys table, §4 sandbox, §5 migration, §8) + SKILL.md updated to the validated INSTANCE path: instantiate `Page` (set `ccd4779c…`, variant `Type`=Basic/Full-screen + `Sandbox`=No/Yes), `slot.insertChild(0, content)` into `"Main content"`/`"Side content"`, then **remove the default `.Content Slot`/`.Side content` placeholders** (validated: main slot ends with only the inserted content). **Sandbox is now a `Page` variant** (`Sandbox=Yes` → built-in yellow border + plashka) — the standalone `Sandbox alert` (`07975654…`, still unpublished) is obsolete/unneeded. The Page's built-in Version=New header is configured from the original: Title + breadcrumb crumb + **relabel its Subheader tabs from the original** (`Label text#4517:0` — validated: Tab_1…5 → Steps/Configurations/Checks Execution Flow) + carry the original action buttons. Hand-building the island from raw frames is now ONLY a logged fallback (file not subscribed). To confirm next full run: header action-BUTTON carry-over + height preservation when resizing the Page instance to the original height (component is natively 1080).

## v3.175.0 — 2026-06-15 (Island layout must be a live `Page` component INSTANCE + slot-fill — never detach, never hand-build)
User feedback: "the skill needs to use an instance of the layout component, not detach it and replace the slot with the needed content". The layout shell must be a live `Page` component instance with content placed into its slot (`slot.insertChild(0, content)`) — NOT a detached instance and NOT an island assembled from raw frames. `island-layout-pattern.md` §2 + §7 + SKILL.md updated: instance+slot is the required primary path; hand-building frames is a FALLBACK allowed ONLY when the `Page` component is unavailable (it must be logged with the reason). 🚩 **Blocker:** the `Page` set (`ccd4779c…`) and `Sandbox alert` (`07975654…`) are still NOT published from Base, so consumer files can't instantiate them (`importComponentSetByKeyAsync` throws "not found") — the skill is forced into the hand-build fallback until they're published. Publishing them is the fix; flagged in the doc + as an ask to the user/design.

## v3.174.0 — 2026-06-15 (Island migration is LOSSLESS: re-skin the header [flip Version], never rebuild — preserve tabs/buttons/toasts)
The first island-migration simulation (v3.173.0) exposed a fundamental flaw in the procedure: it discarded the old header and built a fresh clean `Fullscreen` header with `Subheader=false`/`Key=false` — which **stripped the level-editor's real subheader tabs (Steps / Configurations / Checks Execution Flow) and action buttons (Linked levels / Create level / AI / …)**, and the rebuild also **dropped the toast messages**. User feedback: "during the replacement … the skill lost the toast messages + the subheader with tabs + completely replaced the header and didn't bring the buttons back". Root cause: the old header is the SAME `*Header*` set as the redesign, just `Version=Old` (wrapped in a local `Header-levels`). Fix — `island-layout-pattern.md` §3/§5/§7 + SKILL.md rewritten to a **preservation principle**: a migration RESKINS + REFRAMES, never rebuilds. The header is migrated by **flipping its `*Header*` `Version` Old→New** (`hdr.setProperties({"Version":"New"})`) — verified LOSSLESS (keeps tabs/buttons/title/breadcrumb; height 120→112). `Subheader=false`/`Key=false` is now explicitly ONLY for fresh from-scratch builds, NEVER migration. ALL overlays (toasts/dropdowns) must be preserved (kept + set ABSOLUTE), never dropped. Audit §7 updated: assert header tabs+buttons preserved (count matches old) and overlay count matches old — a breadcrumb-only header or a missing toast = FAIL. Validated end-to-end on a clone (proof: tabs+buttons+toast all preserved, height 800, island 1388, Version=New). NOTE: builds from v3.173.0 and earlier island migrations have stripped headers — re-migrate with this procedure.

## v3.173.0 — 2026-06-15 (Island layout + layout-migration: new reference doc + skill rules, from a 26-mockup migration)
The Dashboard's post-redesign "Island" layout (grey page + flush sidebar + rounded white content card with the header INSIDE it, 8px insets T/R/B, flush-left) is now taught. New reference `island-layout-pattern.md`: full structure + dimensions, all component/token keys (incl. that `Page` set `ccd4779c…` and `Sandbox alert` `07975654…` are unpublished → hand-assemble), the two header types selected by nesting level (1st-level `Generic` 56h / 2nd-level `Fullscreen` breadcrumbs 97h; **✕ in an old header = the 2nd-level signal**), sandbox handling, the full old→island **migration procedure** (in-place or copy), and a **property-level audit checklist**. Validated by migrating 26 KYC level-editor mockups. SKILL.md gains a headline "Island layout + layout migration" rule and the reference-table entry. Hard lessons encoded as rules (each caught a real bug in the migration): (1) **migration preserves the frame's ORIGINAL height** — FIXED height + cascade FILL, never hug (heights drifted 800→922); (2) **sandbox is a STATE detected by a VISIBLE indicator**, never node presence (a hidden "sandbox mode" text in every old header falsely flagged 26 frames); (3) the Fullscreen header is shared with the applicant page — set `Subheader=false`+`Key=false` for config/editor pages or it bleeds tabs/client/tags; (4) recenter content only AFTER container widths settle (else overshoot → x=-492); (5) overlays → `layoutPositioning="ABSOLUTE"` set AFTER auto-layout (no-op otherwise → squishes island to 936); (6) hand-built frames set BOTH layoutSizing axes (`createFrame`=100×100 → plashka shipped 100px instead of 24); (7) **audit by VALUES not presence** — skeleton checks passed present-but-wrong builds repeatedly; (8) migrated output goes on the SOURCE's page, not `figma.currentPage`.

## v3.172.0 — 2026-06-15 (KYB footer button blue #143cff = stale Color collection, not a skill bug; audit must read actual fill)
v3.171 retest: structure was correct (Widget shell, native footer button, Bottom Bar 5d6dd1a8, Window in slot, internal bars hidden — all verified live) but the footer Primary button still rendered **#143cff (blue)** = the user's "old button". Deep variable trace found the cause: the file has **two `Color` variable collections** with the same key `ce42e223` — the current one (`6c3bcce7…/10760:1373`) aliases `…button/primary/default/background-normal` → `semantic/background/neutral/normal` → **#20252c (dark)**, while a stale copy (`…/9943:5950`) aliases it → `semantic/background/blue/normal` → **#143cff (blue)**. This is the WebSDK blue→black redesign; the build bound the STALE collection. **A freshly-created Widget binds the current collection → dark button for free** (proven by test build), so it's pure library staleness (7.56 class), not a skill-logic bug — fix is Assets → Libraries → Update + rebuild. Two skill fixes shipped: (1) the audit must read the button's ACTUAL rendered fill and flag `#143cff` as STALE-LIBRARY — the v3.171 audit falsely reported `#20252c` by resolving the nominal token name against the current collection while the node rendered blue; (2) `Left bar` SLOT must be hidden by reading `slot.visible` (a build left it visible while the audit claimed hidden), and the native footer button is relabeled in place, never replaced. kyb-organisms.md + SKILL.md updated.

## v3.171.0 — 2026-06-15 (KYB WebSDK: REVERTS v3.170 — KYB uses the `Widget` shell, not a bare Window with un-hidden internal bars)
KYB Company-information retest: v3.170's header/footer appeared, but the footer used the OLD blue `#143cff` button and a wrong border (user feedback: "the old button, and the border isn't right"). Live inspection of the canonical originals section (`6980:643402`) proved the pre-v3.171 premise was wrong: **KYB does NOT use a bare `Window / *`; it uses the SAME `Widget` shell as KYC** (set `232e8d4d`, `Type=Content`). The KYB `Window / *` goes into the Widget's content slot (named `"Slot"`, 718-wide — not KYC's `"Content "`), with the Window's OWN internal bars left HIDDEN. v3.170 instead un-hid the Window's internal `Toolbar / Bottom Bar / Desktop` — a *different* footer whose button renders blue. **Verified by test build:** a fresh `Widget` `Type=Content` instance ships its `Bottom Bar` (`5d6dd1a8…`) with a **black `#20252c` Primary button** for free — so the fix is purely structural, not a token/library re-sync. Also documented: KYB `Window / *` sets are local-unpublished (`importComponentSetByKeyAsync` throws "not found"; fetch via a canonical instance's `getMainComponentAsync().parent`), while the Widget shell IS importable by key. `kyb-organisms.md` fully rewritten to the Widget-shell recipe; SKILL.md KYB section + checklist updated; v3.170 "un-hide internal bars" rule reverted.

## v3.170.0 — 2026-06-12 (KYB WebSDK: un-hide the Window's Top Bar + Bottom Bar — header & footer are mandatory)
First websdk-mockup queue test (KYB Company information). The build shipped a bare `Window / Select company` (512×800) with no header and no footer. Root cause: every `Window / *` variant ships its `Toolbar / Top Bar` (nav) AND `Toolbar / Bottom Bar / Desktop` (buttons) children as `visible=false`; the agent created the instance and stopped. New HARD RULE (kyb-organisms.md + SKILL.md KYB section + build checklist): after createInstance, set both bars `visible = true` — a KYB screen MUST have a visible header-with-navigation and footer-with-buttons (compare against a full-screen canonical, not a bare-Window example frame). The bars are already in the instance — toggle visibility, don't insert. User feedback: "the SDK must always have a header with navigation and a footer with buttons, like in the example".
...
Questionnaires sim built the page at 1440 canvas + 257 sidebar (the doc says 1920 + 276 — 4th "agent ignored the corrected layout doc" case) and put "Create questionnaire"/"Import questionnaire" in the *Header*. Canonical (Screen 754) puts both CTAs in the Top Toolbar's `Search + actions` region (right of the search), Header is chrome only. Fixes: questionnaires-pattern.md cleaned of its 1920-vs-1280 internal inconsistency + 🛑 hard rule (1920/276) + CTA-placement note; SKILL.md Header section — table-scoped actions (Create/Import/Export/Add/bulk) belong in the Top Toolbar next to search, not the Header. User feedback: "the buttons that belong to the table ended up in the header instead of being to the right of the table search".
...
4. Agent's Q1 ("Want to check whether the data displays correctly?") + Q2 ("If the button is meant to be in the header, say so — we'll move it") — both banned permission-seek class (v3.121).
...
- "Button X sits in the page title row (the title row inside Content). If it's meant to be in the header, say so — we'll move it" (v3.140)
- "Want to check whether the data displays correctly?" / "Want to verify the data displays correctly?"
- "The table is filled with best-effort data. Want to check it?"
...
**Live sim 2026-05-18 v3.138 Workflow Builder canvas:** clean build (Sidebar 257, Builder Header, Canvas with Top/Right/Bottom bars, 7 curved bezier connectors with correct colors per branch type, 7 nodes from `Node / Canvas` set with correct types). Audit PASS. But user flagged: "flow builder nodes must always have configured=true and empty=false".
...
User question: "why doesn't it look at the original and try to use it to assemble the mockup".
...
**Live sim 2026-05-18 v3.133 on KYB Level editor (file `EP9kfSkVRzQT9jBZe3LqZH`):** agent built pure fabrication:
...
3. Choice-menu paraphrase ("update it yourself via the UI") → v3.108 AUTO-UPDATE
...
- **Banned question patterns added:** "If you need content — say so, I'll add it", "Should I add content inside the cards?", "Want me to add the organism instances now?", "I can fill the cards with organism instances if needed".
...
No-op. User on v3.115; this remote = MINOR mismatch. Tests whether the new inlined pre-flight in sumsub-id-mockup now stops and asks (matching sumsub-mockup behavior), instead of "skipping it as optional".
...
**User feedback:** "until today the pre-flight always worked, there was no problem" + "drop the bullshit about a mandatory Claude restart". Two reverts.
