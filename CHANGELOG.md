...
Batch semantic-token fix on New Layout CM page surfaced a rule gap: design-review treated ANY non-semantic binding on a custom node as a violation, so 6 deliberate `status/green/*` (component-token) bindings on custom badge pills got rebound to their semantic alias targets — Костя corrected: «если применены конкретные переменные компонентов, их не нужно заменять на семантические, менять только базовые или hex значения» (reverted). Token Layer table now has an explicit LEAVE-AS-IS row for component tokens (`status/*`, `components/*`, `button/*`, `field/*`…) applied on custom nodes — only `base/*` and raw hex are violations. Also added batch-fix gotchas: walk per-section (full-page walk drops the MCP transport), data-viz fills stay on base palette (no same-hex semantic exists), bind spacing/radius only on exact-value tokens (no tokens for 1/2/10/20/40/64 spacing, 2/40/100 radius), watch capitalized duplicate collections (`Base/Neutral/90`).

## v3.186.0 — 2026-06-15 (copy/build output must live INSIDE its wrapper section — appendChild, not page-sibling)
The KYC "Configurations" build (v3.185, otherwise correct — Create level now renders on all 5) created a `(made by Claude)` wrapper Section but left the 5 `Page` instances as page-level siblings BESIDE it, so the section looked empty and the user asked "скил считает, что сделал что-то?". A Figma Section does not adopt frames by geometric overlap — you must explicitly `section.appendChild`. Fixes: §5 placement rule (create section → `appendChild` each screen → row layout with fixed gap → confirm section bounds enclose all screens) + §7 audit check (every produced screen is a `child` of the wrapper section, `section.children.length === screenCount`, bounds enclose all; empty section with screens beside it = FAIL). Island migration/build is now complete end-to-end (instance, header incl. carried actions, content in Main, overlays, height, sandbox, placement).
...
User: "нужно, чтобы скил использовал инстанс компонента лейаута, а не детачил его и заменял слот на нужный контент". The layout shell must be a live `Page` component instance with content placed into its slot (`slot.insertChild(0, content)`) — NOT a detached instance and NOT an island assembled from raw frames. `island-layout-pattern.md` §2 + §7 + SKILL.md updated: instance+slot is the required primary path; hand-building frames is a FALLBACK allowed ONLY when the `Page` component is unavailable (it must be logged with the reason). 🚩 **Blocker:** the `Page` set (`ccd4779c…`) and `Sandbox alert` (`07975654…`) are still NOT published from Base, so consumer files can't instantiate them (`importComponentSetByKeyAsync` throws "not found") — the skill is forced into the hand-build fallback until they're published. Publishing them is the fix; flagged in the doc + as an ask to the user/design.
