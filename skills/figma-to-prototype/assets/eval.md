...
| 22 | **The pre-flight version check does not fire in a standalone install.** The skill is a folder under `~/.claude/skills/`, so `${CLAUDE_PLUGIN_ROOT}` is empty: the check must skip in silence. A WebFetch, a version banner or a question about updating at the top of the session is a fail — it would meet every colleague who installed the skill without the plugin | SKILL.md, Pre-flight |
