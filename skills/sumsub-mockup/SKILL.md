...
- **🛑 Use the `Page` component as a live INSTANCE + fill its slot — never detach it, never hand-build the island from raw frames.** `Page` (set `ccd4779c…`) is **published (2026-06-15) — validated**: instantiate it (variant `Type`=Basic/Full screen page, `Sandbox`=No/Yes), `slot.insertChild(0, content)` into `"Main content"`/`"Side content"`, then **remove the default `.Content Slot`/`.Side content` placeholders**. Sandbox = the `Sandbox=Yes` variant (built-in yellow border + plashka) — the standalone `Sandbox alert` is obsolete/unneeded. Configure the Page's built-in Version=New header from the original (Title + crumb + relabel Subheader tabs via `Label text#4517:0` + carry buttons). **🛑 HARD RULE: MANDATORY first step = `importComponentSetByKeyAsync(Page)`; if it resolves you MUST instantiate (hand-building raw frames is BANNED). Hand-build is permitted ONLY if that import THROWS, and the log must paste the error.** Recurring bug: the skill hand-builds even though Page imports fine, calling it "the fallback" — that's an automatic FAIL (the audit re-tests the import; importable + hand-built tree = FAIL). Banned: "I'll hand-build / it's the familiar path / the result looks the same".
- **Two header types by nesting level** (same `*Header*` set, different `Type`): **1st-level** section landing (opened from a sidebar item) → `Generic` (56h) + sidebar 257. **2nd-level** drill-down (detail/editor/sub-config) → `Fullscreen` breadcrumbs header + sidebar 52. A **✕ (Close) in an old header is the signal it's 2nd-level** → use breadcrumbs.
- **🛑 Migration RE-SKINS the existing header — never rebuilds it.** The old header is the SAME `*Header*` set, just `Version=Old` (often wrapped in `Header-levels`); it holds REAL content (subheader tabs, action buttons). Migrate by **flipping its `Version` Old→New** (`hdr.setProperties({"Version":"New"})`) — this preserves tabs/buttons/title/breadcrumb losslessly. NEVER discard it for a fresh clean header, and NEVER `Subheader=false`/`Key=false` on a migrated header (that strips real tabs+buttons — a shipped bug). A migration PRESERVES everything: header content, ALL overlays (toasts/dropdowns), content. (`Subheader=false` clean-config is ONLY for fresh from-scratch builds with junk placeholder tabs.)
...
- **`setProperties` TEXT writes can SILENTLY DROP substrings — always read the text back (v3.199).** Observed 2026-07-27 twice in one build: writing an `*Empty State*` subtitle and an `*Alert*` body through their TEXT properties silently removed the ` U+0441` / ` U+0430` tokens from copy like `Cyrillic “с” U+0441 instead of Latin “c” U+0063` — the Latin codepoints survived, the Cyrillic ones vanished, no error thrown. The build "succeeded" and the delivered copy was wrong. **Rule:** after any `setProperties` TEXT write, re-query the node and assert `node.characters === expected`; for copy containing Unicode codepoints, escapes, or mixed-script text, write `textNode.characters = …` **directly** (allowed on a TEXT inside an instance) and then re-apply range fills / range fonts, because a rewrite resets them.
- **A secondary CTA must be an ACTION, not a no-op (v3.199).** In a zero-result empty state, "Keep original query" does nothing — the operator is already looking at the original query's result. Correct secondary is `Clear search` (start over). "Go back to the original query" is only meaningful in the *auto-corrected* variant, where the results shown are for a different query than the one typed. Also: `*Empty State*`'s 2nd button ships with the default label **`Empty search`** — it is NOT in the audit's banned-strings list, so it survives audit; override it explicitly and check it in the read-back.
