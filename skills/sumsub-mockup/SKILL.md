...
- **🛑 Use the `Page` component as a live INSTANCE + fill its slot — never detach it, never hand-build the island from raw frames.** Instantiate `Page` (set `ccd4779c…`), `slot.insertChild(0, content)`. Hand-building frames is a FALLBACK allowed ONLY when the `Page` set is unavailable (currently unpublished from Base → import throws "not found"). If you must fall back, log why. **Publishing the `Page` + `Sandbox alert` components is the blocker** to doing this canonically — flag it / ask the user.
