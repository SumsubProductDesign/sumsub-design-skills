...
8. **🔴 REVERTED v3.170 (fixed v3.171):** do **NOT** build a bare `Window` and un-hide its internal `Toolbar / Top Bar` / `Toolbar / Bottom Bar / Desktop`. Those internal bars are a *different* footer that renders the OLD blue `#143cff` button and a wrong border (user feedback: "old button / the border isn't right"). Always wrap the Window in the `Widget` shell and keep the Window's OWN internal bars HIDDEN — the Widget's `Bottom Bar` (5d6dd1a8) is the correct black-button footer. See kyb-organisms.md "What v3.170 got wrong".
...
- [ ] **KYB only (v3.171):** built on the `Widget` shell (`232e8d4d`, `Type=Content`), NOT a bare Window. Widget's own Top Bar (718) + `Bottom Bar` (5d6dd1a8) visible. KYB `Window / *` inserted into the `"Slot"` (718) via `insertChild`; Window's OWN internal bars left HIDDEN; `Left bar` SLOT + Image slot hidden (read `slot.visible`). Used the NATIVE footer button (relabel text in place — don't replace it). Did NOT un-hide the Window's internal bars.
- [ ] **KYB button color (v3.172):** read the primary button's ACTUAL fill (`button.fills[0].color`). `#20252c` dark = OK. **`#143cff` blue = STALE-LIBRARY** (old `Color` collection `9943:5950`; WebSDK blue→black redesign not synced) → report "update library in Assets + rebuild", do NOT report PASS or resolve the nominal token name (it masks the staleness).
