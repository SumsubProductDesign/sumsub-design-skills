...
**#8c — PURGE the Slot's DEFAULT content before/after inserting your organism; a leftover default renders BELOW your content.** The `Widget[Type=Content]` master's `Slot` ships a **`Steps NEW`** organism as default content — and it contains the **`SNS ID | Desktop` (Active=True) Sumsub-ID promo block** ("Get verified faster with Sumsub ID" + email input). Real defect (flow run №3, 2026-07-20, Костя: "зачем он в каждый макет засунул блок sumsubid?"): the skill inserted each step's organism but left the default in place → **6 of 9 screens rendered the steps-overview + Sumsub-ID block under their real content**; the canonical shows that block ONLY on the Steps-overview screen (`9196:199933`, "Get verified for Astra"). Run №2 had even flagged and purged the same strays — the purge was ad-hoc, not a recipe step, so it regressed. Rule: when filling ANY slot, remove ALL pre-existing slot children (capture refs BEFORE insertChild, remove by ref after — see the island §5 pattern). Audit: a rendered `Steps NEW` / `SNS ID` / "Get verified faster with Sumsub ID" on any screen other than the Steps-overview step = FAIL.
