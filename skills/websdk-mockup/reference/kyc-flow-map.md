...
A prompt like "build the KYC flow: Welcome → Document Type → Tips → Camera → Liveness → Status" names the ANCHOR steps. The canonical flow contains **more screens between them** — tips before EACH capture step, document back side, processing/pending states. Building only the literally-named screens produces a wrong flow (real defect 2026-07-20: the build had doc-tips assembled by guess and **NO tips before Liveness** — Костя: "использовал не тот экран с типсами перед фотографированием документа, перед лайвнессом его уже не поставил").
